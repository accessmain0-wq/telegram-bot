...
import sys
import tempfile
import fcntl          # ← instance lock အတွက်
...
processed_ids = set()   # ← ဒါနဲ့ message တူတာ နှစ်ခါ မပြေနိုင်တော့ဘူး
...
# ══════════════════════════════════════
#  Single-instance lock  (နှစ်ခါ run မဖြစ်အောင်)
# ══════════════════════════════════════
def acquire_lock():
    # Termux မှာ /tmp မရှိတဲ့အတွက် HOME directory သုံးတယ်
    lock_file = os.path.join(os.path.expanduser("~"), "premium_bot.lock")
    fp = open(lock_file, "w")
    try:
        fcntl.flock(fp, fcntl.LOCK_EX | fcntl.LOCK_NB)
        return fp          # lock ရပြီ
    except IOError:
        print("❌ Bot တစ်ခု ရှိပြီးသားပါ။ နှစ်ခါ run မဖြစ်ရ ခင်ဗျာ။")
        sys.exit(1)


def is_duplicate(message_id: int) -> bool:
    """Message ID စစ်ပြီး ထပ်တူ ဖြေမိတာကို ကာကွယ်တယ်"""
    if message_id in processed_ids:
        return True
    processed_ids.add(message_id)
    # RAM မပြည့်အောင် အဟောင်းတွေ ရှင်းတယ်
...
def cmd_start(msg):
    if is_duplicate(msg.message_id): return   # ← guard
...
def cmd_clear(msg):
    if is_duplicate(msg.message_id): return
    history[msg.from_user.id].clear()
...
# ─────────────────────────────────────
# Text  (command မဟုတ်တာသာ ဖမ်းတယ်)
# ─────────────────────────────────────

@bot.message_handler(
    func=lambda m: m.content_type == "text" and not (m.text or "").startswith("/")
)
def handle_text(msg):
    if is_duplicate(msg.message_id): return   # ← guard
...
if __name__ == "__main__":
    acquire_lock()   # ← Bot တစ်ခုပဲ run နိုင်အောင် lock ချတယ်
    print("═══════════════════════════════════")
